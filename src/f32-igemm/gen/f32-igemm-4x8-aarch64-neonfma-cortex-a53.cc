...
// void xnn_f32_igemm_minmax_ukernel_4x8__asm_aarch64_neonfma_cortex_a53_prfm(
//     size_t mr,                         x0
//     size_t nc,                         x1
//     size_t kc,                         x2 / x0
//     size_t ks,                         x3 / x9
...
// Converted from: src/f32-igemm/gen/f32-igemm-4x8-minmax-asm-aarch64-neonfma-cortex-a53-prfm.S
void Generator::generate(bool prefetch, size_t max_mr, size_t nc_mod_nr, size_t kc, size_t ks, const jit_gemm_params* jit_gemm_params)
{
  ...
  if (prefetch) {
    prfm(kPLDL1KEEP, mem[x20, 0]);
    prfm(kPLDL1KEEP, mem[x20, 64]);
  }
  if (max_mr > 3) {
  ...
  if (max_mr > 3) {
    ...
    csel(x20, x12, x20, kEQ); //   a3 = zero, else += a3 + a_offset
  }
  ...
  if (prefetch) {
    prfm(kPLDL1KEEP, mem[x14, 128]); // Prefetch A1
  }
  ...
  if (prefetch) {
    prfm(kPLDL1KEEP, mem[x15, 128]); // Prefetch A2
  }
  ...
  if (prefetch) {
    prfm(kPLDL1KEEP, mem[x20, 128]); // Prefetch A3
  }
...
xnn_status_t xnn_generate_f32_igemm_ukernel_4x8__aarch64_neonfma_cortex_a53(xnn_code_buffer* code, size_t max_mr, size_t nc_mod_nr, size_t kc, size_t ks, const void* params) {
...
xnn_status_t xnn_generate_f32_igemm_ukernel_4x8__aarch64_neonfma_cortex_a53_prfm(xnn_code_buffer* code, size_t max_mr, size_t nc_mod_nr, size_t kc, size_t ks, const void* params) {
  using namespace xnnpack::aarch64;
  Generator g(code);
  assert(params != nullptr);
