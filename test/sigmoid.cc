...
TEST_F(SigmoidTestQS8, define)
{
  ...
  const float output_scale = 0x1.0p-8f;
  ...
  ASSERT_EQ(xnn_status_success, xnn_define_sigmoid(subgraph, input_id, output_id, /*flags=*/0));

  ASSERT_EQ(subgraph->num_nodes, 1);
  const struct xnn_node* node = &subgraph->nodes[0];
  ASSERT_EQ(node->type, xnn_node_type_sigmoid);
...
TEST_F(SigmoidTestQS8, matches_operator_api)
{
  const int32_t input_zero_point = i8dist(rng);
  const float input_scale = scale_dist(rng);
  const int32_t output_zero_point = INT8_MIN;
  const float output_scale = 0x1.0p-8f;
  ...
  const xnn_status status = xnn_create_sigmoid_nc_qs8(
  ...
  ASSERT_EQ(xnn_status_success, xnn_reshape_sigmoid_nc_qs8(op, batch_size, channels, channels, channels, /*threadpool=*/nullptr));
...
TEST_F(SigmoidTestQU8, matches_operator_api)
{
  ...
  // Call operator API.
  xnn_operator_t op = nullptr;
  const xnn_status status = xnn_create_sigmoid_nc_qu8(
  ...
  ASSERT_EQ(xnn_status_success, xnn_reshape_sigmoid_nc_qu8(op, batch_size, channels, channels, channels, /*threadpool=*/nullptr));
...
TEST_F(SigmoidTestF16, matches_operator_api)
{
  ...
  // Call operator API.
  xnn_operator_t op = nullptr;
  const xnn_status status = xnn_create_sigmoid_nc_f16(/*flags=*/0, &op);
  if (status == xnn_status_unsupported_hardware) {
    GTEST_SKIP();
  }
  ...
  ASSERT_EQ(xnn_status_success, xnn_reshape_sigmoid_nc_f16(op, batch_size, channels, channels, channels, /*threadpool=*/nullptr));
  ASSERT_EQ(xnn_status_success, xnn_setup_sigmoid_nc_f16(op, input.data(), operator_output.data()));
  ASSERT_EQ(xnn_status_success, xnn_run_operator(op, /*threadpool=*/nullptr));
...
TEST_F(SigmoidTestF32, matches_operator_api)
{
  std::uniform_real_distribution<float> f32dist(-25.0f, 25.0f);
  std::generate(input.begin(), input.end(), [&]() { return f32dist(rng); });
  ...
  // Call operator API.
  xnn_operator_t op = nullptr;
  const xnn_status status = xnn_create_sigmoid_nc_f32(/*flags=*/0, &op);
  if (status == xnn_status_unsupported_hardware) {
    GTEST_SKIP();
  }
  ...
  ASSERT_EQ(xnn_status_success, xnn_reshape_sigmoid_nc_f32(op, batch_size, channels, channels, channels, /*threadpool=*/nullptr));
