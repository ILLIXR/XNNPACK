...
TEST_F(EluTestQS8, define)
{
  ...
  ASSERT_EQ(xnn_status_success, xnn_define_elu(subgraph, alpha, input_id, output_id, /*flags=*/0));

  ASSERT_EQ(subgraph->num_nodes, 1);
  const struct xnn_node* node = &subgraph->nodes[0];
  ASSERT_EQ(node->type, xnn_node_type_elu);
  ASSERT_EQ(node->compute_type, xnn_compute_type_qs8);
  ASSERT_EQ(node->params.elu.alpha, alpha);
...
TEST_F(EluTestQS8, matches_operator_api)
{
  const int32_t input_zero_point = i8dist(rng);
  const float input_scale = scale_dist(rng);
  const int32_t output_zero_point = input_zero_point;
  ...
  const xnn_status status = xnn_create_elu_nc_qs8(
    alpha, input_zero_point, input_scale, output_zero_point, output_scale, INT8_MIN,
    INT8_MAX, /*flags=*/0, &op);
  ...
  ASSERT_EQ(xnn_status_success, xnn_reshape_elu_nc_qs8(op, batch_size, channels, channels, channels, /*threadpool=*/nullptr));
  ASSERT_EQ(xnn_status_success, xnn_setup_elu_nc_qs8(op, input.data(), operator_output.data()));
...
TEST_F(EluTestF16, matches_operator_api)
{
  const float alpha = std::uniform_real_distribution<float>(1.0e-4f, 1.0f)(rng);
  ...
  // Call operator API.
  xnn_operator_t op = nullptr;
  const xnn_status status = xnn_create_elu_nc_f16(alpha, /*flags=*/0, &op);
  if (status == xnn_status_unsupported_hardware) {
    GTEST_SKIP();
  }
  ...
  ASSERT_EQ(xnn_status_success, xnn_reshape_elu_nc_f16(op, batch_size, channels, channels, channels, /*threadpool=*/nullptr));
  ASSERT_EQ(xnn_status_success, xnn_setup_elu_nc_f16(op, input.data(), operator_output.data()));
  ASSERT_EQ(xnn_status_success, xnn_run_operator(op, /*threadpool=*/nullptr));
...
TEST_F(EluTestF32, matches_operator_api)
{
  const float alpha = std::uniform_real_distribution<float>(1.0e-4f, 1.0f)(rng);
  std::uniform_real_distribution<float> f32dist(-255.0f, 255.0f);
  ...
  // Call operator API.
  xnn_operator_t op = nullptr;
  const xnn_status status = xnn_create_elu_nc_f32(alpha, /*flags=*/0, &op);
  if (status == xnn_status_unsupported_hardware) {
    GTEST_SKIP();
  }
  ...
  ASSERT_EQ(xnn_status_success, xnn_reshape_elu_nc_f32(op, batch_size, channels, channels, channels, /*threadpool=*/nullptr));
  ASSERT_EQ(xnn_status_success, xnn_setup_elu_nc_f32(op, input.data(), operator_output.data()));
  ASSERT_EQ(xnn_status_success, xnn_run_operator(op, /*threadpool=*/nullptr));
