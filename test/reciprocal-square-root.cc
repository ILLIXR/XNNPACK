...
TEST_F(ReciprocalSquareRootTestF16, define) {
  ...
  ASSERT_EQ(xnn_status_success,
            xnn_define_tensor_value(subgraph, xnn_datatype_fp16, dims.size(),
                                    dims.data(), nullptr, 0,
  ...
  ASSERT_EQ(xnn_status_success,
            xnn_define_reciprocal_square_root(subgraph, input_id, output_id,
                                              /*flags=*/0));

  ASSERT_EQ(subgraph->num_nodes, 1);
  const struct xnn_node* node = &subgraph->nodes[0];
  ASSERT_EQ(node->type, xnn_node_type_reciprocal_square_root);
...
TEST_F(ReciprocalSquareRootTestF32, define) {
  ASSERT_EQ(xnn_status_success, xnn_initialize(/*allocator=*/nullptr));
...
TEST_F(ReciprocalSquareRootTestF16, matches_operator_api) {
  ...
  const xnn_status status =
      xnn_create_reciprocal_square_root_nc_f16(/*flags=*/0, &op);
  ...
  ASSERT_EQ(xnn_status_success, xnn_reshape_reciprocal_square_root_nc_f16(
                                    op, batch_size, channels, channels,
                                    channels, /*threadpool=*/nullptr));
  ASSERT_EQ(xnn_status_success, xnn_setup_reciprocal_square_root_nc_f16(
...
TEST_F(ReciprocalSquareRootTestF32, matches_operator_api) {
  ...
  xnn_operator_t op = nullptr;
  const xnn_status status =
      xnn_create_reciprocal_square_root_nc_f32(/*flags=*/0, &op);
  if (status == xnn_status_unsupported_hardware) {
    GTEST_SKIP();
  }
  ...
  ASSERT_EQ(xnn_status_success, xnn_reshape_reciprocal_square_root_nc_f32(
                                    op, batch_size, channels, channels,
                                    channels, /*threadpool=*/nullptr));
  ASSERT_EQ(xnn_status_success, xnn_setup_reciprocal_square_root_nc_f32(
                                    op, input.data(), operator_output.data()));
  ASSERT_EQ(xnn_status_success, xnn_run_operator(op, /*threadpool=*/nullptr));
