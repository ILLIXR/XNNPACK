...
using Subtract2TestF16 = BinaryTest<uint16_t>;
using Subtract2TestF32 = BinaryTest<float>;

TEST_F(Subtract2TestQS8, define)
{
  ...
  ASSERT_EQ(
    xnn_status_success, xnn_define_quantized_tensor_value(
                          ...
                          output_dims.data(), nullptr, XNN_INVALID_VALUE_ID, /*flags=*/0, &output_id));
  ASSERT_NE(output_id, XNN_INVALID_NODE_ID);

  ASSERT_EQ(
    xnn_status_success,
    xnn_define_subtract(subgraph, output_min, output_max, input1_id, input2_id, output_id, /*flags=*/0));

  ASSERT_EQ(subgraph->num_nodes, 1);
  const struct xnn_node* node = &subgraph->nodes[0];
  ASSERT_EQ(node->type, xnn_node_type_subtract);
...
TEST_F(Subtract2TestQU8, define)
{
...
TEST_F(Subtract2TestF16, define)
{
  ASSERT_EQ(xnn_status_success, xnn_initialize(/*allocator=*/nullptr));
...
TEST_F(Subtract2TestF32, define)
{
  ASSERT_EQ(xnn_status_success, xnn_initialize(/*allocator=*/nullptr));
...
TEST_F(Subtract2TestQS8, matches_operator_api)
{
  const int32_t input1_zero_point = i8dist(rng);
  const float input1_scale = scale_dist(rng);
  const int32_t input2_zero_point = i8dist(rng);
  ...
  // Call operator API.
  ASSERT_EQ(
    xnn_status_success, xnn_create_subtract_nd_qs8(
  ...
  ASSERT_EQ(
    xnn_status_success, xnn_reshape_subtract_nd_qs8(
                          op, input1_dims.size(), input1_dims.data(), input2_dims.size(), input2_dims.data(),
                          /*threadpool=*/nullptr));

  ASSERT_EQ(
    xnn_status_success, xnn_setup_subtract_nd_qs8(
...
TEST_F(Subtract2TestQU8, matches_operator_api)
{
  const int32_t input1_zero_point = u8dist(rng);
  const float input1_scale = scale_dist(rng);
  const int32_t input2_zero_point = u8dist(rng);
  ...
  // Call operator API.
  ASSERT_EQ(
    xnn_status_success, xnn_create_subtract_nd_qu8(
                          input1_zero_point, input1_scale, input2_zero_point, input2_scale, output_zero_point,
                          output_scale, quantized_output_min, quantized_output_max, /*flags=*/0, &op));
  std::unique_ptr<xnn_operator, decltype(&xnn_delete_operator)> auto_op(op, xnn_delete_operator);

  ASSERT_EQ(
    xnn_status_success, xnn_reshape_subtract_nd_qu8(
                          op, input1_dims.size(), input1_dims.data(), input2_dims.size(), input2_dims.data(),
                          /*threadpool=*/nullptr));

  ASSERT_EQ(
    xnn_status_success, xnn_setup_subtract_nd_qu8(
                          op, input1.data(), input2.data(), operator_output.data()));
...
TEST_F(Subtract2TestF16, matches_operator_api)
{
  ...
  // Call operator API.
  const xnn_status status = xnn_create_subtract_nd_f16(output_min, output_max, 0, &op);
  ...
  ASSERT_EQ(
    xnn_status_success, xnn_reshape_subtract_nd_f16(
                          op, input1_dims.size(), input1_dims.data(), input2_dims.size(), input2_dims.data(),
                          /*threadpool=*/nullptr));

  ASSERT_EQ(
    xnn_status_success, xnn_setup_subtract_nd_f16(
                          op, input1.data(), input2.data(), operator_output.data()));
...
TEST_F(Subtract2TestF32, matches_operator_api)
{
  ...
  // Call operator API.
  ASSERT_EQ(xnn_status_success, xnn_create_subtract_nd_f32(output_min, output_max, 0, &op));
  std::unique_ptr<xnn_operator, decltype(&xnn_delete_operator)> auto_op(op, xnn_delete_operator);

  ASSERT_EQ(
    xnn_status_success, xnn_reshape_subtract_nd_f32(
                          op, input1_dims.size(), input1_dims.data(), input2_dims.size(), input2_dims.data(),
                          /*threadpool=*/nullptr));

  ASSERT_EQ(
    xnn_status_success, xnn_setup_subtract_nd_f32(
                          op, input1.data(), input2.data(), operator_output.data()));
